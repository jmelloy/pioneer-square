...
from task_lifecycle import TERMINAL_STATES, finalize_closed_issue, finalize_task
...
# (repo, PR number, head sha). GitHub can refire review_requested for a PR
# that has already been reviewed at the current head commit (e.g. a dismissed
# review being re-requested, or duplicate webhook delivery), and without a
# cooldown the foreman would spin up a duplicate review task each time. A new
# commit changes head sha and always bypasses the cooldown immediately.
# Enforced via LockService (backend/lock_service.py), which is Postgres-backed
# so it holds across backend replicas rather than just within one process.
...
async def _find_issue_conversation_id(
    db, guild_pk: int, repo: str | None, issue_number: int | None
) -> int | None:
    """Best-effort conversation_id (#1271) for an ``issues`` webhook event.

    ``issues`` events carry no PR coordinates for ``_find_task`` to match on,
    so this looks up the most recently created task claiming this issue via
    (issue_repo, issue_number) instead (set at ``assign_task`` time). Returns
    ``None`` when no task has claimed the issue yet, or it predates the
    conversation_id backfill.
    """
...
async def github_webhook(
    ...
    # stamping conversation_id (#1271) best-effort from whichever task claims
    # this issue/PR.
    if event_type == "issues" and repo:
        issue_payload = payload.get("issue")
        if isinstance(issue_payload, dict):
            issue_conversation_id = await _find_issue_conversation_id(
    ...
    # finalize_task's conditional UPDATE is what keeps a task that raced to a
    # terminal state elsewhere from being overwritten here.
    if task_id and event_type == "pull_request" and action == "closed":
        pr = payload.get("pull_request") or {}
        outcome = "done" if pr.get("merged") else "failed"
        res = await finalize_task(
            db, guild_pk=guild_pk, guild_id=guild_id, task_id=task_id, outcome=outcome
        )
        if res.finalized:
            logger.info(
                "github webhook auto-finalized task=%s as %s on PR close guild=%s",
                task_id,
                outcome,
                guild_id,
            )
    ...
    if event_type == "pull_request" and action == "review_requested":
        ...
        if (
            ...
            # review_requested for a PR/commit that was already reviewed recently.
            # Scoped by head sha so a genuinely new commit is reviewed immediately.
            ...
            if not cooldown_acquired:
                logger.info(
                    ...
                    "pr=%s head_sha=%s cooldown_seconds=%s",
                    guild_id,
                    delivery_id,
                    repo,
                    pr_number,
