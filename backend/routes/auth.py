...
async def store_claude_credentials(
    ...
    await authorize_worker_or_member(data.guild_id, token, db)
...
async def guest_login(db: AsyncSession = Depends(get_db)):
    ...
    guild_res = await db.execute(
        select(Guild.guild_id).where(Guild.github_user_id == guest_user_id, _not_deleted).limit(1)
    )
...
async def logout(
    ...
    if credentials:
        await db.execute(delete(UserSession).where(UserSession.token == credentials.credentials))
        await db.commit()
