...
async def list_guilds(
    ...
    result = await db.execute(
        ...
        .outerjoin(
            Agent,
            (Agent.guild_id == Guild.id) & (Agent.type != "foreman") & (Agent.state != "offline"),
        )
...
async def update_guild_member(
    ...
    res = await db.execute(
        select(GuildMember).where(GuildMember.guild_id == guild_pk, GuildMember.user_id == user_id)
    )
...
async def remove_guild_member(
    ...
    await db.execute(
        delete(GuildMember).where(GuildMember.guild_id == guild_pk, GuildMember.user_id == user_id)
    )
    await db.commit()
