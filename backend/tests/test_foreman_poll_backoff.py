...
# ---------------------------------------------------------------------------
# external foreman poll suppression
# ---------------------------------------------------------------------------


def test_suppress_foreman_poll_cancels_task_and_blocks_resets():
    """suppress_foreman_poll must cancel current loop and block future reset spawns."""
    ...
    async def _test():
        ...
        guild = "g-suppress"
        runner._guild_locks.clear()
        runner._guild_last_action_at[guild] = time.monotonic()
        runner._suppressed_poll_guilds.discard(guild)

        old_task = MagicMock()
        old_task.done.return_value = False
        runner._poll_tasks[guild] = old_task

        runner.suppress_foreman_poll(guild)

        old_task.cancel.assert_called_once()
        assert guild not in runner._poll_tasks
        assert guild in runner._suppressed_poll_guilds
        ...
        assert spawned == [], "reset must not start poll while suppressed"
...
def test_resume_foreman_poll_clears_suppression_and_restarts_loop():
    """resume_foreman_poll should clear suppression and delegate to reset_foreman_poll."""
    ...
    async def _test():
        ...
        runner._guild_last_action_at[guild] = time.monotonic()
        runner._poll_tasks.pop(guild, None)
        runner._suppressed_poll_guilds.add(guild)
        ...
        with patch.object(runner, "spawn", side_effect=_fake_spawn):
            runner.resume_foreman_poll(guild)

        assert guild not in runner._suppressed_poll_guilds
        assert len(spawned) == 1, "resume should restart poll via reset_foreman_poll"
        assert runner._poll_tasks[guild] is fake_task
