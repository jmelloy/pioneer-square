...
# Max number of queued human messages per (guild, user) key.
...
async def _emit_foreman_chat(
    ...
    *,
    ...
    ``task_id`` is set whenever the run concerns a task (e.g. a task-complete
    event, or a human reply posted in that task's Discord thread). It both
    badges the WS ``ChatMsg`` so the frontend can show which task a line
    concerns, and routes the Discord mirror to that task's thread so the
    reply lands back where the human is talking.

    ``discord_channel_id`` pins the Discord mirror to one channel outright,
    overriding ``task_id``. Set only for a run triggered by an @-mention
    ...
    (no ``task_id``/``discord_channel_id``) lands in that user's own
...
async def run_foreman_ai(
    ...
    *,
    ...
    """Serialise per-guild runs and delegate to ``_run_foreman_ai``.
    ...
    Runs are keyed on ``(guild_id, user_id)`` — one whole-guild conversation
    per user, regardless of whether the triggering event concerns a specific
    task. Per-conversation isolation for Discord/web UI chat is handled by the
    thread-per-conversation system (see foreman/thread_service.py), not by
    this lock.
...
async def _run_foreman_ai(
    ...
    *,
    ...
    Every run shares the same whole-guild system prompt, tool set, and
    conversation history for a given (guild_id, user_id) — there is no
    per-task scoping of the LLM conversation. ``task_id``, when set, only
    tags outgoing chat lines/messages so the frontend can badge them and so
    the Discord mirror routes into that task's thread (e.g. a task-complete
    event, or a human reply posted in a task's Discord thread); it never
    narrows the workers/tasks visible in this turn or the history loaded.
    """
    proxy_available = has_foreman_proxy(guild_id)
