...
class Worker:
    ...
    async def _execute_task(self, task: dict) -> None:
        ...
        # Each task gets its own agent ID so multiple tasks appear as separate
        # entities in the guild. "t-abc123" → "a-abc123".
        agent_id = "a-" + task_id[2:]
        raw = self.cfg.worker_id[2:].upper()
        split = 2 + sum(ord(c) for c in raw) % 3
        worker_display = f"{raw[:split]}-{raw[split:]}"

        await self._send({
            "type": "join",
            "agentId": agent_id,
            "agentName": f"{worker_display}/{task_id}",
            "agentType": "worker",
            "workerId": self.cfg.worker_id,
        })
        ...
        async def set_state(state: str) -> None:
            await self._send({"type": "agent-state", "agentId": agent_id, "state": state})

        try:
            logger.info("Executing task %s with agent %s: %s", task_id, agent_id, desc[:120])
            ...
            logger.info("Task %s: finished success=%s stop_reason=%s", task_id, success, stop_reason)
            ...
            if success:
                ...
                if push_ok:
                    logger.info("Task %s: opening PR for branch %s", task_id, branch)
                ...
                logger.info("Task %s: done pr_url=%s", task_id, pr_url)
                await self._task_update(
                    task_id, state="done", branch=branch,
                    prUrl=pr_url or "", finishedAt=finished_at,
                )
                await self._send({
                    "type": "task-complete",
                    "workerId": self.cfg.worker_id,
            ...
            await self._send({"type": "agent-state", "agentId": agent_id, "state": "offline"})
