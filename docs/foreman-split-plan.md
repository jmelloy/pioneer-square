...
For how the embedded Foreman scopes a run's conversation history to a thread
and treats task_id as metadata rather than a separate context, see
[foreman-per-task-context.md](foreman-per-task-context.md) — that's internal
to the backend and orthogonal to the proxy boundary described here.
...
history, tool execution, broadcasts, per-(guild, user) run locking, and periodic polling.
...
loader, or run-lock supervisor of its own.
